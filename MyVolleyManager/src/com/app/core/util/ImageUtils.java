...
public class ImageUtils {
    ...
    /**
     * 图片配置
     * @param drawableId 默认图/错误图
     * @return
     */
    public static DisplayImageOptions getOptions(int drawableId) {
        if(drawableId == -1)
            return new DisplayImageOptions.Builder().showImageOnLoading(null).showImageForEmptyUri(null).showImageOnFail(null)
                .cacheInMemory(true).cacheOnDisc(true).resetViewBeforeLoading(true).imageScaleType(ImageScaleType.IN_SAMPLE_INT)
                .bitmapConfig(Bitmap.Config.RGB_565).build();
        return new DisplayImageOptions.Builder().showImageOnLoading(drawableId).showImageForEmptyUri(drawableId)
            .showImageOnFail(drawableId).cacheInMemory(true).cacheOnDisc(true).resetViewBeforeLoading(true)
