/*
 * Copyright 2012 Kevin Sawicki <kevinsawicki@gmail.com> Licensed under the Apache License, Version 2.0 (the "License"); you may not
...
public class DecodeBitmapTask extends AsyncTask<Void, Void, Bitmap> {
    ...
    /**
     * Create task to decode the bitmap at the specified path to the specified maximum width and height
     * @param maxWidth
     * @param maxHeight
     * @param path
     */
    ...
    protected Bitmap decode() {
        ...
        if(options.outWidth > maxWidth || options.outHeight > maxHeight)
            scale=
                Math.max(Math.round((float)options.outHeight / (float)maxHeight),
        ...
        if(Log.isLoggable(TAG, DEBUG)) {
            if(decoded == null) {
                ...
                Log.d(TAG, "Decoded to " + decoded.getWidth() + "x" + decoded.getHeight() + " from max size: " + maxWidth + "x"
                    + maxHeight + " using scale:" + scale + " and byte count:" + getByteCount(decoded));
        }
        return decoded;
    }
